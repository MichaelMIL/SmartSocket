/*
 * Relay Control UI Component
 *
 * Provides a UI component for controlling a relay with a color-changing button.
 * Red = OFF, Green = ON
 * Uses an object-oriented approach with a struct to encapsulate state.
 */

#include "relay_control_ui.h"
#include "relay_hardware.h"
#include <stdbool.h>
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
#include <inttypes.h>
#include "lvgl.h"
#include "esp_log.h"
#include "esp_timer.h"

static const char *DEFAULT_TAG = "relay_ui";

// Last creation error, readable for diagnostics when no console is attached
static const char *s_last_error = "none";

const char *relay_control_ui_last_error(void)
{
    return s_last_error;
}

/**
 * @brief Control the relay hardware based on state
 *
 * @param ui Pointer to the relay control UI object
 * @param state true = ON, false = OFF
 */
static void control_relay_hardware(relay_control_ui_t *ui, bool state)
{
    if (ui == NULL || ui->hardware == NULL) {
        return;
    }

    relay_hardware_set_state(ui->hardware, state);
}

/**
 * @brief Animation callback for progress bar value update
 */
static void progress_bar_anim_cb(void *var, int32_t value)
{
    lv_obj_t *bar = (lv_obj_t *)var;
    lv_bar_set_value(bar, value, LV_ANIM_OFF);
}

/**
 * @brief Format time as MM:SS string
 *
 * @param seconds Total seconds
 * @param buffer Output buffer
 * @param buffer_size Size of the output buffer (at least 16 bytes recommended)
 */
static void format_time_string(uint32_t seconds, char *buffer, size_t buffer_size)
{
    uint32_t minutes = seconds / 60;
    uint32_t secs = seconds % 60;
    snprintf(buffer, buffer_size, "%02" PRIu32 ":%02" PRIu32, minutes, secs);
}

/**
 * @brief Update the current consumption display
 *
 * @param ui Pointer to the relay control UI object
 */
static void update_current_display(relay_control_ui_t *ui)
{
    if (ui == NULL || ui->current_label == NULL || ui->current_container == NULL) {
        return;
    }

    // Read current from hardware if available
    float current = 0.0f;
    if (ui->hardware != NULL) {
        current = relay_hardware_read_current(ui->hardware);
    }

    // Format current string with arrow pointing to button
    char current_str[20];
    if (ui->is_left_side) {
        snprintf(current_str, sizeof(current_str), "← %.2f A", current);
    } else {
        snprintf(current_str, sizeof(current_str), "%.2f A →", current);
    }
    lv_label_set_text(ui->current_label, current_str);

    // Always show current container (even when relay is OFF, it will show 0.00 A)
    lv_obj_clear_flag(ui->current_container, LV_OBJ_FLAG_HIDDEN);

    // Change color based on current value for better visibility
    lv_color_t text_color, bg_color;
    if (current > 0.1f) {
        // Current flowing - use bright green text with darker green background
        text_color = lv_color_hex(0x00FF00);
        bg_color = lv_color_hex(0x004400);
    } else {
        // No current or very low - use gray text with dark background
        text_color = lv_color_hex(0x808080);
        bg_color = lv_color_hex(0x202020);
    }

    lv_obj_set_style_text_color(ui->current_label, text_color, LV_PART_MAIN);
    lv_obj_set_style_bg_color(ui->current_container, bg_color, LV_PART_MAIN);
    lv_obj_set_style_border_color(ui->current_container, text_color, LV_PART_MAIN);
}

/**
 * @brief Update the timer display
 *
 * @param ui Pointer to the relay control UI object
 */
static void update_timer_display(relay_control_ui_t *ui)
{
    if (ui == NULL || ui->timer_label == NULL) {
        return;
    }

    if (ui->state && ui->time_remaining > 0) {
        char time_str[16];
        format_time_string(ui->time_remaining, time_str, sizeof(time_str));
        lv_label_set_text(ui->timer_label, time_str);
        lv_obj_clear_flag(ui->timer_label, LV_OBJ_FLAG_HIDDEN);

        // Update progress bar animation
        if (ui->progress_bar != NULL) {
            // Calculate progress percentage (0-100)
            uint32_t elapsed = RELAY_TIMER_DURATION_SECONDS - ui->time_remaining;
            int32_t progress = (int32_t)((elapsed * 100) / RELAY_TIMER_DURATION_SECONDS);
            if (progress < 0) progress = 0;
            if (progress > 100) progress = 100;

            // Change color based on remaining time: green -> yellow -> red
            uint32_t remaining_percent = 100 - progress;
            lv_color_t bar_color;
            if (remaining_percent > 50) {
                bar_color = lv_color_hex(0x00FF00);
            } else if (remaining_percent > 20) {
                bar_color = lv_color_hex(0xFFFF00);
            } else {
                bar_color = lv_color_hex(0xFF0000);
            }
            lv_obj_set_style_bg_color(ui->progress_bar, bar_color, LV_PART_INDICATOR);

            // Animate the progress bar value change (smooth transition)
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, ui->progress_bar);
            lv_anim_set_values(&a, lv_bar_get_value(ui->progress_bar), progress);
            lv_anim_set_time(&a, 1000);  // 1 second smooth animation
            lv_anim_set_exec_cb(&a, progress_bar_anim_cb);
            lv_anim_start(&a);

            lv_obj_clear_flag(ui->progress_bar, LV_OBJ_FLAG_HIDDEN);
        }

        ESP_LOGD(ui->tag, "Timer display updated: %s", time_str);
    } else {
        lv_label_set_text_static(ui->timer_label, "");
        lv_obj_add_flag(ui->timer_label, LV_OBJ_FLAG_HIDDEN);

        // Hide progress bar
        if (ui->progress_bar != NULL) {
            lv_obj_add_flag(ui->progress_bar, LV_OBJ_FLAG_HIDDEN);
            lv_bar_set_value(ui->progress_bar, 0, LV_ANIM_OFF);
        }
    }
}

/**
 * @brief Update the button appearance based on relay state
 *
 * @param ui Pointer to the relay control UI object
 */
static void update_button_appearance(relay_control_ui_t *ui)
{
    if (ui == NULL || ui->button == NULL || ui->label == NULL) {
        return;
    }

    // Format the label text with the relay name
    char label_text[32];
    const char *display_name = (ui->name != NULL && strlen(ui->name) > 0) ? ui->name : "RELAY";

    lv_obj_set_style_bg_color(ui->button, ui->state ? BUTTON_ON_COLOR : BUTTON_OFF_COLOR, LV_PART_MAIN);
    snprintf(label_text, sizeof(label_text), "%s %s", display_name, ui->state ? "ON" : "OFF");
    lv_label_set_text(ui->label, label_text);
    ESP_LOGD(ui->tag, "Relay UI: %s", ui->state ? "ON (Green)" : "OFF (Red)");

    update_timer_display(ui);
}

/**
 * @brief Timer callback - counts down and sets flag for UI update
 * NOTE: This runs in esp_timer task context - DO NOT call LVGL functions here!
 */
static void timer_callback(void *arg)
{
    relay_control_ui_t *ui = (relay_control_ui_t *)arg;

    if (ui == NULL || ui->time_remaining == 0) {
        return;
    }

    ui->time_remaining--;
    ui->update_needed = true;  // Signal that UI update is needed

    if (ui->time_remaining == 0) {
        // Timer expired - turn relay off
        ESP_LOGI(ui->tag, "Timer expired - will turn relay OFF");
        ui->state = false;
        // Control hardware immediately (safe to call from timer context)
        control_relay_hardware(ui, false);
        // Stop the countdown (safe to call from the timer's own callback)
        if (ui->timer != NULL) {
            esp_timer_stop(ui->timer);
        }
    }
}

/**
 * @brief LVGL timer callback - safely updates UI from LVGL context
 */
static void lvgl_timer_cb(lv_timer_t *timer)
{
    relay_control_ui_t *ui = (relay_control_ui_t *)lv_timer_get_user_data(timer);

    if (ui == NULL) {
        return;
    }

    // Check if update is needed (set from esp_timer callback or start_timer/stop_timer)
    if (ui->update_needed) {
        ui->update_needed = false;  // Clear flag

        // Now safe to call LVGL functions since we're in LVGL timer context
        update_timer_display(ui);

        // If timer just started, reset progress bar
        if (ui->time_remaining == RELAY_TIMER_DURATION_SECONDS && ui->progress_bar != NULL) {
            lv_bar_set_value(ui->progress_bar, 0, LV_ANIM_OFF);
            lv_obj_set_style_bg_color(ui->progress_bar, lv_color_hex(0x00FF00), LV_PART_INDICATOR);  // Start green
        }

        // If timer expired, also update button appearance
        if (ui->time_remaining == 0 && !ui->state) {
            update_button_appearance(ui);
        }
    }

    // Check if state change update is needed (set from HTTP handler or other non-LVGL contexts)
    if (ui->state_update_needed) {
        ui->state_update_needed = false;  // Clear flag

        // Now safe to call LVGL functions since we're in LVGL timer context
        update_button_appearance(ui);
        update_current_display(ui);
    }
}

/**
 * @brief LVGL timer callback for current reading updates
 */
static void current_timer_cb(lv_timer_t *timer)
{
    relay_control_ui_t *ui = (relay_control_ui_t *)lv_timer_get_user_data(timer);

    if (ui == NULL) {
        return;
    }

    // Update current display (safe to call from LVGL timer context)
    update_current_display(ui);
}

/**
 * @brief Start (or restart) the auto-off countdown
 *
 * Safe to call from any context - never touches LVGL directly.
 *
 * @param ui Pointer to the relay control UI object
 */
static void start_timer(relay_control_ui_t *ui)
{
    if (ui == NULL || ui->timer == NULL) {
        return;
    }

    // Restart if already running (esp_timer_stop returns an error when
    // the timer is not running - that is fine here)
    esp_timer_stop(ui->timer);

    ui->time_remaining = RELAY_TIMER_DURATION_SECONDS;

    // Signal that UI update is needed (will be handled by LVGL timer callback)
    ui->update_needed = true;

    esp_err_t ret = esp_timer_start_periodic(ui->timer, 1000000); // 1 second period
    if (ret != ESP_OK) {
        ESP_LOGE(ui->tag, "Failed to start timer: %s", esp_err_to_name(ret));
        return;
    }

    ESP_LOGI(ui->tag, "Auto-off timer started: %d seconds", RELAY_TIMER_DURATION_SECONDS);
}

/**
 * @brief Stop the auto-off countdown
 *
 * Safe to call from any context - never touches LVGL directly.
 *
 * @param ui Pointer to the relay control UI object
 */
static void stop_timer(relay_control_ui_t *ui)
{
    if (ui == NULL) {
        return;
    }

    if (ui->timer != NULL) {
        esp_timer_stop(ui->timer);
    }

    ui->time_remaining = 0;
    // Signal that UI update is needed (will be handled by LVGL timer callback)
    ui->update_needed = true;
}

/**
 * @brief Button click event callback
 */
static void relay_button_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    relay_control_ui_t *ui = (relay_control_ui_t *)lv_event_get_user_data(e);

    if (ui == NULL) {
        return;
    }

    if (code == LV_EVENT_LONG_PRESSED) {
        // Long press: Turn ON without the auto-off timer
        if (!ui->state) {
            ui->state = true;
            control_relay_hardware(ui, ui->state);

            // Set flag to prevent the trailing CLICKED event from toggling back off
            ui->long_press_active = true;

            update_button_appearance(ui);

            if (ui->state_change_cb != NULL) {
                ui->state_change_cb(ui, ui->state);
            }

            ESP_LOGI(ui->tag, "Relay button long-pressed, state: ON (no timer)");
        }
    } else if (code == LV_EVENT_CLICKED) {
        // A CLICKED event always follows LONG_PRESSED on release - ignore it
        if (ui->long_press_active) {
            ui->long_press_active = false;
            ESP_LOGD(ui->tag, "Ignoring CLICKED event after long press");
            return;
        }

        // Short click: Normal toggle behavior with timer
        ui->state = !ui->state;
        control_relay_hardware(ui, ui->state);

        if (ui->state) {
            start_timer(ui);
        } else {
            stop_timer(ui);
        }

        update_button_appearance(ui);

        if (ui->state_change_cb != NULL) {
            ui->state_change_cb(ui, ui->state);
        }

        ESP_LOGI(ui->tag, "Relay button clicked, state: %s", ui->state ? "ON" : "OFF");
    }
}

relay_control_ui_t *relay_control_ui_create(lv_obj_t *parent, const char *tag, const char *name, lv_align_t align, int16_t x_offset, int16_t y_offset, relay_hardware_t *hardware)
{
    if (parent == NULL) {
        ESP_LOGE(DEFAULT_TAG, "Cannot create relay control UI: parent is NULL");
        return NULL;
    }

    relay_control_ui_t *ui = (relay_control_ui_t *)calloc(1, sizeof(relay_control_ui_t));
    if (ui == NULL) {
        ESP_LOGE(DEFAULT_TAG, "Failed to allocate memory for relay control UI");
        s_last_error = "Failed to allocate memory";
        return NULL;
    }

    ui->tag = (tag != NULL) ? tag : DEFAULT_TAG;
    ui->name = (name != NULL) ? name : "RELAY";
    ui->hardware = hardware;  // Can be NULL for UI-only operation

    // Sync UI state with hardware state if hardware is available
    ui->state = (hardware != NULL) ? relay_hardware_get_state(hardware) : false;

    // Create the auto-off countdown timer once; started/stopped on demand
    const esp_timer_create_args_t timer_args = {
        .callback = &timer_callback,
        .arg = ui,
        .name = "relay_timer"
    };
    if (esp_timer_create(&timer_args, &ui->timer) != ESP_OK) {
        ESP_LOGE(ui->tag, "Failed to create countdown timer");
        s_last_error = "Failed to create countdown timer";
        goto fail;
    }

    // Create button with label
    ui->button = lv_button_create(parent);
    if (ui->button == NULL) {
        ESP_LOGE(ui->tag, "Failed to create button");
        s_last_error = "Failed to create button";
        goto fail;
    }
    lv_obj_set_size(ui->button, BUTTON_WIDTH_PX, BUTTON_HEIGHT_PX);
    lv_obj_align(ui->button, align, x_offset, y_offset);

    ui->label = lv_label_create(ui->button);
    if (ui->label == NULL) {
        ESP_LOGE(ui->tag, "Failed to create label");
        s_last_error = "Failed to create label";
        goto fail;
    }
    lv_obj_center(ui->label);

    // Create timer label above the button
    ui->timer_label = lv_label_create(parent);
    if (ui->timer_label == NULL) {
        ESP_LOGE(ui->tag, "Failed to create timer label");
        s_last_error = "Failed to create timer label";
        goto fail;
    }
    lv_obj_align_to(ui->timer_label, ui->button, LV_ALIGN_OUT_TOP_MID, TIMER_LABEL_X_OFFSET_PX, TIMER_LABEL_Y_OFFSET_PX);
    lv_label_set_text_static(ui->timer_label, "");
    lv_obj_set_style_text_align(ui->timer_label, TIMER_LABEL_TEXT_ALIGN, LV_PART_MAIN);
    lv_obj_set_style_text_color(ui->timer_label, TIMER_LABEL_TEXT_COLOR, LV_PART_MAIN);
    // Initially hidden (shows when relay is ON and timer is running)
    lv_obj_add_flag(ui->timer_label, LV_OBJ_FLAG_HIDDEN);

    // Create progress bar for animated countdown visualization
    ui->progress_bar = lv_bar_create(parent);
    if (ui->progress_bar == NULL) {
        ESP_LOGE(ui->tag, "Failed to create progress bar");
        s_last_error = "Failed to create progress bar";
        goto fail;
    }
    lv_obj_align_to(ui->progress_bar, ui->button, LV_ALIGN_OUT_BOTTOM_MID, PROGRESS_BAR_X_OFFSET_PX, PROGRESS_BAR_Y_OFFSET_PX);
    lv_obj_set_size(ui->progress_bar, PROGRESS_BAR_WIDTH_PX, PROGRESS_BAR_HEIGHT_PX);
    lv_bar_set_value(ui->progress_bar, 0, LV_ANIM_OFF);
    lv_bar_set_range(ui->progress_bar, 0, 100);
    lv_obj_set_style_bg_color(ui->progress_bar, lv_color_hex(0x00FF00), LV_PART_INDICATOR);  // Starts green
    lv_obj_add_flag(ui->progress_bar, LV_OBJ_FLAG_HIDDEN);

    // Determine if button is on left or right side based on alignment (for current arrow direction)
    ui->is_left_side = (align == LV_ALIGN_TOP_LEFT || align == LV_ALIGN_LEFT_MID || align == LV_ALIGN_BOTTOM_LEFT);
    bool is_right_side = (align == LV_ALIGN_TOP_RIGHT || align == LV_ALIGN_RIGHT_MID || align == LV_ALIGN_BOTTOM_RIGHT);

    // Create container for current display with arrow
    ui->current_container = lv_obj_create(parent);
    if (ui->current_container == NULL) {
        ESP_LOGE(ui->tag, "Failed to create current container");
        s_last_error = "Failed to create current container";
        goto fail;
    }
    lv_obj_set_size(ui->current_container, 80, 30);
    lv_obj_set_style_bg_opa(ui->current_container, LV_OPA_80, LV_PART_MAIN);
    lv_obj_set_style_bg_color(ui->current_container, lv_color_hex(0x202020), LV_PART_MAIN);
    lv_obj_set_style_border_width(ui->current_container, 2, LV_PART_MAIN);
    lv_obj_set_style_border_color(ui->current_container, lv_color_hex(0x808080), LV_PART_MAIN);
    lv_obj_set_style_radius(ui->current_container, 5, LV_PART_MAIN);
    lv_obj_set_style_pad_all(ui->current_container, 4, LV_PART_MAIN);

    // Position container next to the button, on the side facing the screen center
    if (ui->is_left_side) {
        lv_obj_align_to(ui->current_container, ui->button, LV_ALIGN_OUT_RIGHT_MID, 0, -15);
    } else if (is_right_side) {
        lv_obj_align_to(ui->current_container, ui->button, LV_ALIGN_OUT_LEFT_MID, 0, 15);
    } else {
        lv_obj_align_to(ui->current_container, ui->button, LV_ALIGN_OUT_BOTTOM_MID, CURRENT_LABEL_X_OFFSET_PX, CURRENT_LABEL_Y_OFFSET_PX);
    }

    // Create current consumption label inside the container
    ui->current_label = lv_label_create(ui->current_container);
    if (ui->current_label == NULL) {
        ESP_LOGE(ui->tag, "Failed to create current label");
        s_last_error = "Failed to create current label";
        goto fail;
    }
    lv_obj_center(ui->current_label);
    lv_label_set_text_static(ui->current_label, "0.00 A");
    lv_obj_set_style_text_align(ui->current_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_set_style_text_color(ui->current_label, CURRENT_LABEL_TEXT_COLOR, LV_PART_MAIN);

    // Create LVGL timer for safe UI updates from non-LVGL contexts
    // (periodically checks the update_needed/state_update_needed flags)
    ui->lvgl_timer = lv_timer_create(lvgl_timer_cb, 100, ui);
    if (ui->lvgl_timer == NULL) {
        ESP_LOGE(ui->tag, "Failed to create LVGL timer");
        s_last_error = "Failed to create LVGL timer";
        goto fail;
    }
    lv_timer_set_repeat_count(ui->lvgl_timer, -1);

    // Create LVGL timer for current reading updates
    ui->current_timer = lv_timer_create(current_timer_cb, CURRENT_UPDATE_INTERVAL_MS, ui);
    if (ui->current_timer == NULL) {
        ESP_LOGE(ui->tag, "Failed to create current timer");
        s_last_error = "Failed to create current timer";
        goto fail;
    }
    lv_timer_set_repeat_count(ui->current_timer, -1);

    // Set initial appearance
    update_button_appearance(ui);
    update_current_display(ui);

    // Click toggles with timer, long press turns ON without timer
    lv_obj_add_event_cb(ui->button, relay_button_cb, LV_EVENT_CLICKED, ui);
    lv_obj_add_event_cb(ui->button, relay_button_cb, LV_EVENT_LONG_PRESSED, ui);

    // Make button style more prominent
    lv_obj_set_style_radius(ui->button, 10, LV_PART_MAIN);
    lv_obj_set_style_shadow_width(ui->button, 10, LV_PART_MAIN);
    lv_obj_set_style_shadow_color(ui->button, lv_color_hex(0x808080), LV_PART_MAIN);
    lv_obj_set_style_shadow_opa(ui->button, LV_OPA_50, LV_PART_MAIN);

    ESP_LOGI(ui->tag, "Relay control UI object created");

    return ui;

fail:
    relay_control_ui_delete(ui);
    return NULL;
}

void relay_control_ui_delete(relay_control_ui_t *ui)
{
    if (ui == NULL) {
        return;
    }

    if (ui->timer != NULL) {
        esp_timer_stop(ui->timer);
        esp_timer_delete(ui->timer);
        ui->timer = NULL;
    }

    if (ui->lvgl_timer != NULL) {
        lv_timer_del(ui->lvgl_timer);
        ui->lvgl_timer = NULL;
    }

    if (ui->current_timer != NULL) {
        lv_timer_del(ui->current_timer);
        ui->current_timer = NULL;
    }

    // Deleting a container also deletes its children (labels)
    if (ui->current_container != NULL) {
        lv_obj_del(ui->current_container);
        ui->current_container = NULL;
        ui->current_label = NULL;
    }

    if (ui->progress_bar != NULL) {
        lv_obj_del(ui->progress_bar);
        ui->progress_bar = NULL;
    }

    if (ui->timer_label != NULL) {
        lv_obj_del(ui->timer_label);
        ui->timer_label = NULL;
    }

    if (ui->button != NULL) {
        lv_obj_del(ui->button);
        ui->button = NULL;
        ui->label = NULL;
    }

    free(ui);
}

bool relay_control_ui_get_state(const relay_control_ui_t *ui)
{
    if (ui == NULL) {
        return false;
    }
    return ui->state;
}

void relay_control_ui_set_state(relay_control_ui_t *ui, bool state)
{
    if (ui == NULL) {
        return;
    }

    bool old_state = ui->state;
    ui->state = state;

    control_relay_hardware(ui, ui->state);

    // Start timer when turning ON, stop when turning OFF
    if (ui->state && !old_state) {
        start_timer(ui);
    } else if (!ui->state && old_state) {
        stop_timer(ui);
    }

    // Signal that UI update is needed (will be handled by LVGL timer callback)
    // DO NOT call LVGL functions directly here - this may be called from an HTTP handler
    ui->state_update_needed = true;

    // Notify state change callback (must not call LVGL functions directly either)
    if (ui->state_change_cb != NULL) {
        ui->state_change_cb(ui, ui->state);
    }
}

void relay_control_ui_toggle(relay_control_ui_t *ui)
{
    if (ui == NULL) {
        return;
    }
    relay_control_ui_set_state(ui, !ui->state);
}

void relay_control_ui_set_state_change_callback(relay_control_ui_t *ui, relay_state_change_cb_t cb, void *arg)
{
    if (ui == NULL) {
        return;
    }
    ui->state_change_cb = cb;
    ui->state_change_cb_arg = arg;
}

lv_obj_t *relay_control_ui_get_button(relay_control_ui_t *ui)
{
    if (ui == NULL) {
        return NULL;
    }
    return ui->button;
}

relay_hardware_t *relay_control_ui_get_hardware(relay_control_ui_t *ui)
{
    if (ui == NULL) {
        return NULL;
    }
    return ui->hardware;
}
